...
we define predicates `IsKrasnerNormed K L` for arbitary field extensions
...
## TODO
Defined a valued version `IsKrasnerValued K L` and prove `IsKrasnerValued.of_complete`
under the assumption `[IsValExtension vK.v vL.v] [hv : vK.v.RankOne] [hv : vL.v.RankOne]
[Algebra.IsAlgebraic K L]`.
The problem is that `IsValExtension` does NOT implies `NormedAlgebra`,
since the norm may differ by an exponential.

-/

-- This lemma should be put into the file `SpectalNormUnique` after `SpectalNormUnique` get merged into Mathlib.
lemma NormedAlgebra.norm_eq_spectralNorm (K : Type*) {L : Type*} [NontriviallyNormedField K]
...
/--
Give a field extension `L / K` with a norm on `L`, `IsKrasnerNormed K L` is the abstraction
